/*
    Realizar un programa que gestione los datos de stock de una tienda de
   comestibles, la información a recoger será: Código del producto, precio,
   cantidad en stock, Stock Mínimo. La tienda dispone de N productos distintos.
    ...
    if (code >= 0)
    {
      for (int i = 0; i < position; i++)
      {
        if (products[i].cod == code)
        {
          ...
          clear();
          topLine();
          content("se decremento en 1 el stock de nombre: %s",
                  products[i].name.str);
          midLine();
    ...
    if (code >= 0)
    {
      for (int i = 0; i < position; i++)
      {
        if (products[i].cod == code)
        {
          ...
          midLine();
          content("Precio aterior: %f, incerte el nuevo precio",
                  products[i].price);
          midLine();
          inputLine();
          float price;
          ...
          clear();
          topLine();
          content("Precio del producto %s modificado, ahora es $%f",
                  products[i].name.str, products[i].price);
...
void analizeStock(Product *products)
{
  ...
  for (int i = 0; i < position; i++)
  {
    ...
    {
      midLine();
      content("nombre: %s, stock: %d,codico %d, precio: %f, stock minimo: %d",
              products[i].name.str, products[i].stock, products[i].cod,
              products[i].price, products[i].minStock);
    }
  }
  midLine();
