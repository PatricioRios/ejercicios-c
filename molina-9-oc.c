...
int main() {
  ...
  WindowOption opts[7] = {
      ...
      {"Ordenar los elementos pares de la lista de Menor a Mayor y los impares de Menor a Mayor",
       2, opt_order_numbers},
...
void opt_calculate_prom(void *context){
  ...
  clear();
  topLine();
  content("El promedio de todos los numeros es: %f",(float)ac/(float)stack->len);
  midLine();
  waiting();
...
void opt_add_number_on_bottom(void *context){
  ...
  clear();
  topLine();
  content("Dame el numero que queres añadir en el final de la pila (cantidad de numeros %d)",stack->len);
  midLine();
  inputLine();
  scanf("%d",&number);
